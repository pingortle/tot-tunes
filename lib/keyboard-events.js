...
const EVENT_STRUCTURE_BYTES = 16
const EVENT_STRUCTURES_PER_EVENT = 3
const EVENT_CLUSTER_BYTES = EVENT_STRUCTURE_BYTES * EVENT_STRUCTURES_PER_EVENT
const KEYBOARD_EVENT_CLUSTER_INDEX = 1
const KEYBOARD_EVENT_CLUSTER_POSITION =
  KEYBOARD_EVENT_CLUSTER_INDEX * EVENT_STRUCTURE_BYTES
...
function toEvents (buffer) {
  ...
  for (let i = 0; i < buffer.length; i += EVENT_CLUSTER_BYTES) {
    ...
    const eventData = clusterData.slice(
      KEYBOARD_EVENT_CLUSTER_POSITION,
      KEYBOARD_EVENT_CLUSTER_POSITION + EVENT_STRUCTURE_BYTES
    )
